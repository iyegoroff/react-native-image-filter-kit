...
static inline float4 exclusion(const float4 dst, const float4 src) {
  float4 out;
...
uchar4 RS_KERNEL blendImage(uchar4 dst, uint32_t x, uint32_t y) {
  return rsPackColorTo8888(exclusion(
...
uchar4 RS_KERNEL blendColor(uchar4 dst, uint32_t x, uint32_t y) {
  return rsPackColorTo8888(exclusion(rsUnpackColor8888(dst), srcColor));
}
