...
static inline float4 color(const float4 dst, const float4 src) {
  float4 srcDstAlpha = src * dst.a;
  float4 out;

  out.a = src.a + (1.0f - src.a) * dst.a;
  out.rgb = setLuminance(srcDstAlpha.rgb, srcDstAlpha.a, dst.rgb * src.a);
...
uchar4 RS_KERNEL blendColor(uchar4 dst, uint32_t x, uint32_t y) {
  return rsPackColorTo8888(color(rsUnpackColor8888(dst), srcColor));
}
